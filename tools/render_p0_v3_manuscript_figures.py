...
def load_core_curve_rows(core_root: Path) -> dict[int, list[dict[str, Any]]]:
    ...
    for budget in range(1, 7):
        ...
        if len(paths) != 5:
            raise ValueError(
                f"expected five complete core files for B={budget}, found {len(paths)}"
            )
...
def render_effects(rows: list[dict[str, Any]], summary: dict[str, Any], output: Path) -> None:
    ...
    for fold in range(5):
        ...
        estimates.append(
            (f"Fold {fold + 1}", float(values.mean()), _paired_interval(values, 20260810 + fold))
        )
    pooled = _metric(summary, 6, "independent_confirmation_source_rollout", "T")["paired_vs_source"]
    estimates.append(
        ("All 230", float(pooled["paired_mean_difference"]), tuple(pooled["paired_bootstrap_95ci"]))
    )
    ...
    image = matrix.imshow(
        counts, cmap="Blues", vmin=0, vmax=int(counts.max()), origin="lower", aspect="equal"
    )
...
def render_waterfall(summary: dict[str, Any], output: Path) -> None:
    ...
    figure, axes = plt.subplots(
        2, 1, figsize=(6.7, 3.35), sharex=True, sharey=True, constrained_layout=True
    )
    ...
    for axis, (policy, counts) in zip(axes, values.items(), strict=True):
        ...
        for index, (start, height) in enumerate(zip(starts, heights, strict=True)):
            if index in (0, 2, 4):
                axis.text(
                    index, start + height + 0.10, f"{start + height:.3f}", ha="center", fontsize=7
                )
            elif height > 0.03:
                axis.text(
                    index,
    ...
    delta_ft = (ic_f["policy_mean"] - ic["policy_mean"]) - (
        source_f["source_mean"] - source["source_mean"]
    )
    figure.text(
        0.50,
        0.50,
...
def render_budget_curve(summary: dict[str, Any], core_root: Path, output: Path) -> None:
    ...
    for axis, metric, title in zip(
        axes.flat[:3],
        ("T", "F", "D"),
        ("Complete-pool $T$", "Selected-history $F$", "Provisional $D$"),
        strict=True,
    ):
        for policy, (label, color, style) in policies.items():
            values = [
                _metric(summary, int(budget), policy, metric)["policy_mean"]
                if policy != "source_margin"
                else _metric(summary, int(budget), policy, metric)["source_mean"]
        ...
        axis.set(
            ...
            ylabel="Mean confirmations/system",
            title=title,
        )
        _finish(axis)
    ...
    for metric, label, color in (
        ...
        for budget in budgets:
            paired = _metric(
                summary, int(budget), "independent_confirmation_source_rollout", metric
            )["paired_vs_source"]
    ...
    axis.set(
        xticks=budgets,
        xlabel="Query budget $B$",
        ylabel="IC-SARR minus source",
        title="Paired differences (95% CI)",
    )
    ...
    for budget in budgets:
        paired = _metric(
            summary, int(budget), "independent_confirmation_source_rollout", "wall_seconds"
        )["paired_vs_source"]
        mean = float(paired["paired_mean_difference"])
        ci = paired["paired_bootstrap_95ci"]
        means.append(mean)
        ...
        upper.append(float(ci[1]) - mean)
    axis.errorbar(
        budgets, means, yerr=[lower, upper], color=NEGATIVE, marker="s", ms=3.2, lw=1.2, capsize=2.2
    )
    axis.axhline(0, color=SOURCE, lw=0.8)
    axis.set(
        xticks=budgets,
        xlabel="Query budget $B$",
        ylabel="Seconds/system",
        title="Additional wall time (95% CI)",
    )
    _finish(axis)
...
def _direct_metric(
    summary: dict[str, Any], budget: int, policy: str, baseline: str, metric: str
) -> dict[str, Any]:
    return summary["direct_pairwise_contrasts"][str(budget)][f"{policy}_vs_{baseline}"]["metrics"][
        metric
    ]["direct_paired"]


def render_objective_first_budget_curve(
    e32_summary: dict[str, Any], objective_summary: dict[str, Any], output: Path
) -> None:
    """Render the primary same-posterior objective/solver/cost figure."""
    budgets = np.arange(1, 7)
    objective_rows = objective_summary["paired_budget_curve"]
    objective_means = np.asarray(
        [row["paired_mean_difference"] for row in objective_rows], dtype=float
    )
    objective_ci = np.asarray([row["paired_bootstrap_95ci"] for row in objective_rows], dtype=float)
    ...
    solver_means = np.asarray([row["paired_mean_difference"] for row in solver_rows])
    solver_ci = np.asarray([row["paired_bootstrap_95ci"] for row in solver_rows])
    ...
    figure, axes = plt.subplots(1, 3, figsize=(7.0, 2.75), constrained_layout=True)
    objective_axis, solver_axis, cost_axis = axes
    objective_axis.axhline(0, color=SOURCE, lw=0.8)
    objective_axis.errorbar(
        budgets,
        objective_means,
        yerr=[objective_means - objective_ci[:, 0], objective_ci[:, 1] - objective_means],
        color=POSITIVE,
        marker="o",
        ms=3.7,
        lw=1.2,
    ...
    objective_axis.set(
        xticks=budgets,
        xlabel="Query budget $B$",
        ylabel=r"$\Delta T$ / system",
        title="Objective contrast",
    )
    auc = objective_summary["auc"]["difference"]
    objective_axis.text(
        0.04,
        0.97,
        f"AUC {auc['paired_mean_difference']:+.3f}\n95% CI [{auc['paired_bootstrap_95ci'][0]:+.3f}, {auc['paired_bootstrap_95ci'][1]:+.3f}]",
        transform=objective_axis.transAxes,
        va="top",
        fontsize=6.3,
        color=INK,
    )
    _finish(objective_axis)

    solver_axis.axhline(0, color=SOURCE, lw=0.8)
    solver_axis.errorbar(
        budgets,
        solver_means,
        yerr=[solver_means - solver_ci[:, 0], solver_ci[:, 1] - solver_means],
        color=DELTA,
        marker="o",
        ms=3.7,
        lw=1.2,
    ...
    solver_axis.set(
        ...
        xlabel="Query budget $B$",
        ylabel=r"$\Delta T$ / system",
        title="Solver contrast",
    )
    solver_axis.text(
        0.04,
        0.97,
        f"B=6 {solver_means[-1]:+.3f}\n95% CI [{solver_ci[-1, 0]:+.3f}, {solver_ci[-1, 1]:+.3f}]",
    ...
    cost_axis.axhline(0, color=SOURCE, lw=0.8)
    cost_colors = (DELTA, NEGATIVE)
    for (label, rows), color in zip(cost_pairs.items(), cost_colors, strict=True):
        means = np.asarray([row["paired_mean_difference"] for row in rows], dtype=float)
        ci = np.asarray([row["paired_bootstrap_95ci"] for row in rows], dtype=float)
        cost_axis.errorbar(
            budgets,
            means,
            yerr=[means - ci[:, 0], ci[:, 1] - means],
            color=color,
            marker="o",
            ms=3.3,
            lw=1.1,
    ...
    _finish(cost_axis)
    figure.suptitle("Same-posterior objective and solver contrasts (230 systems)", fontsize=9.5)
    _save(figure, output)
...
def render_ablation(summary: dict[str, Any], output: Path) -> None:
    ...
    axis.set(
        xticks=positions,
        xticklabels=labels,
        ylabel="B=6 paired $\\Delta T$ / system",
        title="Targeted MatPES mechanism audit (5 folds, 230 systems)",
    )
...
def render_calibration(calibration_path: Path, output: Path) -> None:
    ...
    calibration.plot(
        x, t, color=POSITIVE, marker="o", ms=3.8, lw=1.3, label="Realized $T$ advantage"
    )
    calibration.plot(
        x, f, color=CAUTION, marker="s", ms=3.8, lw=1.3, label="Realized $F$ advantage"
    )
    calibration.set(
        xlabel="Predicted rollout advantage (decile mean)",
        ylabel="Realized advantage",
    ...
    strata.set(
        xticks=positions, xticklabels=labels, ylabel="Realized advantage", title="Gate strata"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--e32-summary",
        type=Path,
        help="completed E32-A summary for the objective-first main figures",
    )
    parser.add_argument(
        "--objective-summary",
        type=Path,
        help="evaluator-only Delta-Hull minus target-margin summary",
    )
    ...
    if (args.e32_summary is None) != (args.objective_summary is None):
        raise SystemExit("--e32-summary and --objective-summary must be provided together")
    if args.e32_summary is not None:
        e32_summary = json.loads(args.e32_summary.read_text(encoding="utf-8"))
        objective_summary = json.loads(args.objective_summary.read_text(encoding="utf-8"))
        render_delta_hull_waterfall(e32_summary, args.output_dir / "matpes_dft_waterfall.pdf")
        render_objective_first_budget_curve(
            e32_summary,
            objective_summary,
            args.output_dir / "matpes_budget_curve.pdf",
        )
    else:
    ...
    render_direct_mechanism_comparisons(
        direct_summary, args.output_dir / "matpes_direct_mechanism_comparisons.pdf"
    )
