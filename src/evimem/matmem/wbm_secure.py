...
class CorrectedPhaseEntry:
    ...
    def __post_init__(self) -> None:
        ...
        if not math.isclose(
            ...
            raise ValueError(
                "corrected phase entry energy must be total energy in eV, not eV/atom"
            )
...
class AppendOnlyWBMEventLog:
    ...
    def __init__(self, path: Path) -> None:
        ...
        try:
            ...
            raise FileExistsError("WBM event log is append-only and cannot overwrite a run") from exc
    ...
    def append_action(
        ...
        *,
        ...
        previous = None
        if self._records:
            previous = self._records[-1].model_dump().get(
                "reveal_checksum",
        ...
        *,
        ...
        for item in items:
            ...
            if not math.isclose(
                ...
                raise ValueError(
                    "oracle total energy and corrected phase entry energy disagree"
                )
...
class CompositionHullState:
    ...
    def _diagram(self, selected: Iterable[CorrectedPhaseEntry]) -> object:
        ...
        return PhaseDiagram(
            [*self._initial_entries, *(item.entry for item in selected)]
        )
    ...
    def _formation_hull_energy_per_atom(self, composition: object) -> float:
        ...
        elemental_reference = sum(
            float(fraction)
            * float(self._phase_diagram.el_refs[Element(element)].energy_per_atom)
    ...
    def snapshot(
        ...
        *,
        ...
        return query.hull_snapshot.model_copy(
            update={
                "snapshot_id": (
                    f"causal:{self.source_version}:{round_index}:{query.query_id}"
                ),
    ...
    def selected_final_stability(self) -> dict[str, bool]:
        ...
        return {
            item.query_id: bool(
                diagram.get_e_above_hull(item.entry, allow_negative=True) <= 1e-8
            )
    ...
    def oracle_final_stability(
        ...
        return {
            query_id: bool(
                diagram.get_e_above_hull(by_id[query_id].entry, allow_negative=True)
                <= 1e-8
            )
...
class SecureWBMRunner:
    ...
    def run(self, *, oracle_budget: float) -> WBMRunResult:
        ...
        while remaining:
            ...
            causal = (
                query.hull_distance(
                    revealed.observation.corrected_formation_energy_ev_per_atom
                )
                <= query.stability_threshold_ev_per_atom
            )
