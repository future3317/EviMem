...
* **Causal horizon F**: selected candidates that are stable in the hull
  containing only the current revealed phase records and the outcomes selected
  along that simulated continuation. Unselected sampled outcomes are never
  inserted into this hull.
