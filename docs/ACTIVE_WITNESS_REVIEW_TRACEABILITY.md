...
| Retention target/solver mismatch | Enumerate every subset with size at most `K`; interval conflict makes exact-size-`K`/one-swap reasoning invalid | `boundary.py`, `acquisition.py` | random exhaustive equivalence plus a two-old-witness simultaneous-removal construction | Frozen NO-GO paper retains the audited mismatch; post-freeze code uses the true Eq. 13 objective with `O(n²K2^K)` direct runtime |
