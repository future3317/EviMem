...
Status: post-E44 exact separation and post-E45 budget-one secondary audit.
...
| “IC-SARR only improves oracle labels.” | MatPES reports `+0.161/system` oracle-final development gain; MAD reports AUC `+0.2240`, final-causal `+0.0260` with CI crossing zero. | Final-causal superiority is unsupported. | Always pair any oracle claim with the final-causal limitation. |
| “The method is too expensive.” | MAD primary utility is `-0.6334`; wall-time AUC rises `+8.5739`. | No cost-aware superiority or runtime advantage. | Keep cost co-primary; do not use the E45 secondary timings to soften E43. |
...
| “Numerical integration creates the effect.” | Existing MC64/MC256 action-parity probes and independent IC gate constrain the primary result. | The proposed MC512/1024/8192 disagreement-state audit is not yet complete. | Do not imply that high-precision state-level verification has been done. |
...
- Do not report the E45 budget-one checkpoint as a curve, a significance test,
  or a new main table.
- If the MC state audit or remaining ladder is resumed, freeze an explicit
  resource allocation and report incomplete work as incomplete.
